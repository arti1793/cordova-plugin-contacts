...
@objc(CDVContacts) class CDVContacts: CDVPlugin, CNContactViewControllerDelegate, CNContactPickerDelegate {
    ...
    func checkPermission(_ command: CDVInvokedUrlCommand){
        ...
        if status == .notDetermined {
            CNContactStore().requestAccess(for: CNEntityType.contacts, completionHandler: {(_ granted: Bool, _ error: Error?) -> Void in
            self.commandDelegate.send(CDVPluginResult(status: CDVCommandStatus_OK, messageAs: granted), callbackId: callbackId)
            })
        } else{
            self.commandDelegate.send(CDVPluginResult(status: CDVCommandStatus_OK, messageAs: status == CNAuthorizationStatus.authorized), callbackId: callbackId)}
    }
